...
void MainWindow::on_pushButton_3_clicked()
{
    ...
    if(db.open()){
        ...
        zapytanie.exec("SELECT id_pracownika,id_przedmiotu FROM przydzial_zajec");
        int licznik=0;
        while(zapytanie.next())
        {
            ...
            przyciski[licznik]->setFixedSize(250,250);
            przyciski[licznik]->setSizePolicy(QSizePolicy::Fixed,QSizePolicy::Fixed);
            layout->addWidget(przyciski[licznik]);
            QObject::connect(przyciski[licznik],SIGNAL (clicked()),this,SLOT(ustaw_przyciski()));
            przyciski[licznik]->setCheckable(true);
            przyciski[licznik]->setAutoExclusive(true);

            QColor color(rand()%256,rand()%256,rand()%256);
            QString qss = QString("background-color: %1").arg(color.name());
            przyciski[licznik]->setStyleSheet(qss);
            licznik++;
        }
...
void MainWindow::ustaw_przyciski()
{
    ...
    if( button != nullptr )
    {
        ...
        for(int i=0; i<4;i++){
            przyciski[i]->setChecked(false);
            przyciski[i]->setAutoExclusive(true);
        }
...
{
    ui->tableWidget->setItem(row,column,new QTableWidgetItem("cos"));
}
