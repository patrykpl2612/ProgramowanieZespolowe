...
void MainWindow::on_actionExport_to_PDF_triggered()
{
    ...
    printer.setOutputFileName("plan.pdf");

    QTextDocument doc;

    QString text('<table style="border: 1px solid black;');
    text.append("<tr>");
    text.append("<th>").append("</th>");
    for (int i = 0; i < tabelki[0]->columnCount(); i++) {
        text.append("<th>").append(tabelki[0]->horizontalHeaderItem(i)->data(Qt::DisplayRole).toString()).append("</th>");
    }

    text.append("</tr></thead>");
    text.append("<tbody>");
    for (int i = 0; i < tabelki[0]->rowCount(); i++) {
        text.append("<tr>");
        text.append("<td>").append(QString::number(i+godzina_poczatkowa)).append("-").append(QString::number(i+godzina_poczatkowa+1)).append("</td>");
        for (int j = 0; j < tabelki[0]->columnCount(); j++) {
            QTableWidgetItem *item = tabelki[0]->item(i, j);
            if (!item || item->text().isEmpty()) {
                tabelki[0]->setItem(i, j, new QTableWidgetItem(""));
            }
            text.append("<td>").append(tabelki[0]->item(i, j)->text()).append("</td>");
        }
        text.append("</tr>");
    }
    ...
    for (int j = 0; j < tabelki[0]->columnCount(); j++)
    {
        text.append("<td>").append("                    ").append("</td>");
    }
