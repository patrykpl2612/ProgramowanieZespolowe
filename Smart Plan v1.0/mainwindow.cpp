...
void MainWindow::on_actionConnect_to_database_triggered()
{
    ...
    while(zapytanie.next())
    {
        ...
        zapytanie_3.exec(z);
        zapytanie_3.next();
        QString imie = zapytanie_2.value(0).toString();
        QString etykieta = QString(imie[0] + ". " + zapytanie_2.value(1).toString()+"\n"+zapytanie_3.value(0).toString());
...
void MainWindow::wczytaj_plan_zajec(int licznik)
{
    ...
    while(zapytanie.next())
    {
        ...
        tabelki[licznik]->setItem(row-godzina_poczatkowa, id_dnia.indexOf(column), item);
        qDebug()<<id_dnia;
        qDebug()<<column<<id_dnia[column-1];
...
void MainWindow::on_actionExport_to_PDF_triggered()
{
    ...
    for(int k=0; k<tabelki.size(); k++)
    {
        ...
        printer.setOutputFileName("Plan numer "+QString::number(k)+".pdf");

        QTextDocument doc;
        //qDebug()<<k;
        QString text("<table border=1>");
        text.append("<tr>");
        text.append("<th>").append("</th>");
        for (int i = 0; i < tabelki[k]->columnCount(); i++) {
            text.append("<th>").append(tabelki[k]->horizontalHeaderItem(i)->data(Qt::DisplayRole).toString()).append("</th>");
        }


        text.append("</tr></thead>");
        text.append("<tbody>");
        for (int i = 0; i < tabelki[k]->rowCount(); i++) {
            text.append("<tr>");
            text.append("<td>").append(QString::number(i+godzina_poczatkowa)).append("-").append(QString::number(i+godzina_poczatkowa+1)).append("</td>");
            for (int j = 0; j < tabelki[k]->columnCount(); j++) {
                QTableWidgetItem *item = tabelki[k]->item(i, j);
                if (!item || item->text().isEmpty()) {
                    tabelki[k]->setItem(i, j, new QTableWidgetItem(""));
                }
                text.append("<td>").append(tabelki[k]->item(i, j)->text()).append("</td>");
            }
            text.append("</tr>");
        }
