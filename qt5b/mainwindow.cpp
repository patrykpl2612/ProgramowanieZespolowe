...
void MainWindow::utworz_przycisk_przydzialu(int id, QString etykieta)
{
    ...
    // ustawiamy stały rozmiar
    przyciski_przydzialow->button(id)->setFixedSize(250,250);
    przyciski_przydzialow->button(id)->setSizePolicy(QSizePolicy::Fixed,QSizePolicy::Fixed);
...
{
    if(db.open()){
        if(przyciski_przydzialow->button(1)->isChecked()){
            ui->tableWidget->setItem(row,column,new QTableWidgetItem);
            ui->tableWidget->item(row,column)->setBackground(przyciski_przydzialow->button(1)->palette().button().color());
        }
    }
}
