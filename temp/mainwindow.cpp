...
void MainWindow::ustaw_parametry_tabeli()
{
    ...
    //bgBrush = ui->tableWidget->item(0,0)->background();
...
void MainWindow::utworz_przycisk_przydzialu(int id, QString etykieta)
{
    ...
    ///losowanie koloru z przedzdialow
    QColor color;
    ...
    ///ustawiamy kolor przycisku oraz fonty
    QString css = QString("background-color: %1; color: black;").arg(color.name());
    przyciski_przydzialow->button(id)->setStyleSheet(css);
    QFont font = przyciski_przydzialow->button(id)->font();
    ///uncomment juezeli bold
    //font.setBold(true);
    przyciski_przydzialow->button(id)->setFont(font);
}
...
void MainWindow::ustaw_przyciski(int id, bool stan)
{
    ...
    if(stan)
    {
        ...
        QColor color(przyciski_przydzialow->button(id)->palette().background().color());
        QString css = QString("background-color: %1; color: black; background-color: green;").arg(color.name());
        przyciski_przydzialow->button(id)->setStyleSheet(css);
    }
...
void MainWindow::on_actionAdd_to_database_2_triggered()
{
...
void MainWindow::on_pushButton_clicked()
{
    ...
    if(!db.isOpen())
    {
        ...
        for(int i = 0; i<8; i++)
        {
            QString etykieta = QString(licznik);
            utworz_przycisk_przydzialu(licznik, etykieta);
            id_przydzialu.append(licznik);
            licznik++;
        }
