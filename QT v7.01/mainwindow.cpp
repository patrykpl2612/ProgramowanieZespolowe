    ...
    ui(new Ui::MainWindow)
{
    ...
    connect(ui->tableWidget, SIGNAL(customContextMenuRequested(const QPoint &)),this, SLOT(usun_komorke(const QPoint &)));
...
bool MainWindow::polacz_z_baza_danych()
{
    ...
    if (db.open())
    {
        ...
        while(zapytanie.next())
        {
            ...
            connect(tabelki[licznik], SIGNAL(cellClicked(int ,int )), this, SLOT(add_to_table(int , int )));
...
{
    delete ui->tableWidget->itemAt(pos);
}

void MainWindow::add_to_table(int row, int column)
{
    QTableWidgetItem * item =  tabelki[ui->tabWidget->currentIndex()-1]->item(row,column);
    if(!item)
    {
        item = new QTableWidgetItem();
        tabelki[ui->tabWidget->currentIndex()-1]->setItem(row, column, item);
    }
